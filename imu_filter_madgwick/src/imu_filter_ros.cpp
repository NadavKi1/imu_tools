      ...
      tf_listener_(tf_buffer_)
{
    ...
    if (!nh_private_.getParam("use_mag", use_mag_)) use_mag_ = false; //true;
    ...
    ROS_INFO("world frame = %d, (%s)", world_frame_, world_frame.c_str());
    ...
    imu_subscriber_.reset(new ImuSubscriber(
        nh_, ros::names::resolve("imu1"), queue_size));
...
void ImuFilterRos::publishFilteredMsg(const ImuMsg::ConstPtr& imu_msg_raw)
{
    ...
    if (publish_debug_topics_)
    {
        ...
        // publishOrientationFiltered(imu_msg); // TODO: Remove RS
    }
}
